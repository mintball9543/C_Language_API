...
LRESULT CALLBACK WndProc(HWND hwnd, UINT imsg, WPARAM wParam, LPARAM lParam) {
	...
	switch (imsg) {
	case WM_CREATE:
		updown = CreateWindow(UPDOWN_CLASS, 0, WS_CHILD | WS_VISIBLE , 100, 100, 50, 50, hwnd, 0, hinst, 0);
		SendMessage(updown, UDM_SETPOS, 0, 0);
		break;
	case WM_KEYDOWN:
		memset(pos, 0, 10);
		itoa(SendMessage(updown, UDM_GETPOS, 0, 0),pos,10);
